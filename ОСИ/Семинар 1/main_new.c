...
b = open("a.txt", 1);         // 1. В b запишется -1.
							  // 2. В b запишется -1.
							  
m = write( b, "hello\n", 12); // 1. В m запишется -1, так как файловый дескриптор - это неотрицательное значение.
							  // 2. В m запишется -1, так как файловый дескриптор - это неотрицательное значение.
							  
c = open("a.txt", 0);         // 1. В c запишется 3, как первый не занятый номер в списке
                              // дескрипторов файлов, ассоциированных с процессом
							  // 2. В c запишется 3, как первый не занятый номер в списке
                              // дескрипторов файлов, ассоциированных с процессом
							  
d = read( c, buf, 3);         // 1. В d запишется 3, как количество прочитанных байт из файла a.txt
							  // 2. В d запишется 3, как количество прочитанных байт из файла a.txt

n = read( d, buf, 10);        // 1. В n запишется 3, как количество прочитанных байт из файла a.txt,
							  // поскольку в файл изначально было записано 6 символов и уже прочитано 3,
							  // то 6-3=3 символа осталось 
							  // 2. В n запишется 3, как количество прочитанных байт из файла a.txt,
							  // поскольку в файл изначально было записано 6 символов и уже прочитано 3,
							  // то 6-3=3 символа осталось 

p = read( c, &buf[d+n], 6);   // 1. В p запишется 0, так как файл a.txt достиг своего конца.
							  // 2. В p запишется 0, так как файл a.txt достиг своего конца.
