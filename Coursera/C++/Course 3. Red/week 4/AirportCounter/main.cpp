...
public:
  ...
  AirportCounter(TIterator begin, TIterator end): AirportCounter()
  {
      ...
      {
          data[static_cast<std::size_t>(*curr)].first = *curr;
          Insert(*curr);
      }
  }

  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const { return data[static_cast<std::size_t>(airport)].second;}

  // добавить данный элемент
  void Insert(TAirport airport) { data[static_cast<std::size_t>(airport)].second++;}

  // удалить одно вхождение данного элемента
  void EraseOne(TAirport airport) { data[static_cast<std::size_t>(airport)].second--;}

  // удалить все вхождения данного элемента
  void EraseAll(TAirport airport) { data[static_cast<std::size_t>(airport)].second=0;}
  ...
  const auto& GetItems() const & { return data; }


private:
  static constexpr std::size_t size_ = static_cast<std::size_t>(TAirport::Last_);
  std::array<Item, size_> data;
};
