...
ALTER TABLE [Актёр] ADD 
	...
	CONSTRAINT [Если_есть_ФИО_то_оно_не_пусто] CHECK (nullOrNonEmpty([ФИО]) = 1),
	CONSTRAINT [Если_есть_Город_то_он_не_пуст] CHECK (nullOrNonEmpty([Город_проживания]) = 1)
...
ALTER TABLE [Фильм] ADD 
	...
	CONSTRAINT [Если_есть_Название_то_оно_не_пусто] CHECK (nullOrNonEmpty([Название]) = 1),
	CONSTRAINT [Если_есть_Жанр_то_он_не_пуст] CHECK (nullOrNonEmpty([Жанр]) = 1),
...
ALTER TABLE [Киностудия] ADD 
	...
	CONSTRAINT [Если_есть_Название_то_оно_не_пусто] CHECK (nullOrNonEmpty([Название]) = 1),
	CONSTRAINT [Если_есть_Город_расположения_то_он_не_пуст] CHECK (nullOrNonEmpty([Город_расположения]) = 1)

GO

-- При удалении контракта выставляем данные в NULL, чтобы  сохранить информацию о контракте --
-- Когда обе стороны контракта в NULL, то срабатывает триггер на удаление контратка --

ALTER TABLE [Контракты] ADD
	CONSTRAINT [Контракты_при_удалении_Актёра] FOREIGN KEY ([Код_киностудии]) REFERENCES [Киностудия]([Код_киностудии])
		ON DELETE SET NULL
		ON UPDATE SET NULL ,
	CONSTRAINT [Контракты_при_удалении_Киностудии] FOREIGN KEY ([Код_актёра]) REFERENCES [Актёр]([Код_актёра])
		ON DELETE SET NULL
		ON UPDATE SET NULL

-- TO DO: TRIGGER --
GO
-- Ограничения на удаление/измнение зависимых сущностей при удалении родителей --
